...
class EdalizeToolchain:
    def __init__(self, toolchain):
        ...
        # FIXME: used only by sim. Refactor
        self._backend = None

    def build(self, platform, fragment, build_dir, build_name, run, verilog_args={},
        **kwargs):

        # FIXME: used only by sim; introduce run() to handle running?
        ...
        if build:
            ...
            elif self._toolchain == "verilator":
                tool_options = {
                    "mode":                 "cc",
                    "libs":                 [],
                    "verilator_options":    [],
            ...
            # FIXME: sim uses "run" switch for running simulation and "build" for generating & building;
            #        other targets use "run" for building.
            if self._toolchain == "verilator":
                run = True
