...
unsigned int sdcard_response[5];
...
int sdcard_wait_cmd_done(void) {
	...
	while (1) {
		...
		if(check_counter > CHECK_LOOPS_PRINT_THRESHOLD) { putchar('\n'); return 0xFF; }
		if (cmdevt & 0x1) {
			if (cmdevt & 0x4)
				return SD_TIMEOUT;
			else if (cmdevt & 0x8)
...
int sdcard_wait_response(void) {
	...
	for(i = 0; i < 5; i++) {
	  ...
	  sdcard_response[4 - i] = r;  // NOTE: this is "backwards" but sticking with this because it's compatible with CSR32
	}
#else
