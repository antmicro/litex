...
static void write_inc_dq_delay(int moduel) {
	/* Increment DQ delay */
	ddrphy_wdly_dq_inc_write(1);
	cdelay(100);
}

static void write_rst_dq_delay(int moduel) {
...
static void write_inc_dqs_delay(int moduel) {
	/* Increment DQS delay */
	ddrphy_wdly_dqs_inc_write(1);
	cdelay(100);
}

static void write_rst_dqs_delay(int moduel) {
#if defined(SDRAM_PHY_USDDRPHY) || defined(SDRAM_PHY_USPDDRPHY)
	/* Reset DQS delay */
	while (ddrphy_wdly_dqs_inc_count_read() != 0) {
		ddrphy_wdly_dqs_inc_write(1);
...
static void sdram_deselectd(int module, int dq_line) {
#ifdef SDRAM_PHY_SUBCHANNELS
	if (module < SDRAM_PHY_MODULES/2)
...
static void sdram_leveling_action(int module, int dq_line, action_callback action) {
	...
	/* Un-select module */
	sdram_deselectd(module, dq_line);
}
